...
If you like to download and organize your media, then you want to know how to manage your own series in Anikku.
...
1. In the location you specified as your storage location (e.g., `/Anikku/`), there should be a `local` folder. Place correctly structured series inside that (e.g. `/Anikku/local/`).
...
Note that a single folder or archive file is treated as a single chapter. For example, Anikku will not automatically split an `EPUB` file containing multiple chapters into separate chapters within the app.
...
Anikku will see three chapters in a single series.
